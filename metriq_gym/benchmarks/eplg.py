...
class EPLG(Benchmark[EPLGData, EPLGResult]):
    ...
    def _build_circuits(
        ...
        list, list, list[int], list[list[tuple[int, int]]], str, list[str]
    ]:
    ...
    def dispatch_handler(self, device: "QuantumDevice") -> EPLGData:
        ...
        # Count 2Q gates for both input and transpiled circuits
        input_two_qubit_gate_counts = [
            count_gates(c).two_qubit for c in input_circuits
        ]
        transpiled_two_qubit_gate_counts = [
            count_gates(c).two_qubit for c in transpiled_circuits
        ]
